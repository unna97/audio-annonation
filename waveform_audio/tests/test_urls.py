...
class TestURLs:
    ...
    def test_save_annotations_url(self):
        ...
        assert resolve(url).func.view_class.__name__ == "SaveAnnotationsView"
    ...
    @pytest.mark.xfail(reason="Need to fix api root")
    def test_api_url(self):
        url = reverse("api:api-root")

        assert resolve(url).namespace == "api"

    @pytest.mark.xfail(reason="Need to fix media url")
